...
static int run(void *d) {
    ...
    for(j=0;(j<count_rules(data->rules) && DEFINITIVE_RULE!=RULE_MATCH);j++){
        ...
        if( !is_dependant_rule(data->rules,j) && (type & NORMAL_SCORE) && has_zero_score(data->rules,j)==1 
        ...
        if( type & NORMAL_RULE) {
                ...
                //pthread_mutex_t *dependant_mutex;
...
// Obtain list of registered prescedulers
        ...
        printf("Allocating memmory for preschedulers\n");
        // Allocate memory for function information, if any
        if (num_cl_exts > 0) {
...
static void stop(void *d) {
	...
	// Release function data, if any
     ...
     //NO; EL RULESET HABRA QUE CARGARSELO AL FINAL
