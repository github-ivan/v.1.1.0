...
int parse_ini(char *path, ini_file **config_values){
    ...
    inifile=create_ini_file();

    if(inifile==NULL) return CONFIG_FAIL;

    if(ae_load_eml_to_memory(path,&text)<=0) return CONFIG_FAIL;
    ...
    while(start_pointer[count]!='\0'){
        ...
        if(status==BEGIN_SECTION && start_pointer[count]=='['){
            wblprintf(LOG_CRITICAL,"INI_PARSER","Error, section incorrect\n");
            *config_values=NULL;
            return CONFIG_FAIL;
        }
        if(status==BEGIN_SECTION && (start_pointer[count]==']') ){
        ...
        if(status==BEGIN_ATTRIBUTE && start_pointer[count]=='\\' 
           && start_pointer[count+1]!='\n'){
                wblprintf(LOG_CRITICAL,"INI_PARSER","Error, \\ must precede \n");
                *config_values=NULL;
                return CONFIG_FAIL;
        }
        if( status==BEGIN_ATTRIBUTE  && (start_pointer[count]==';'
...
int get_attribute_values_ini(ini_file *file,char *section_name,
