...
typedef void (*dns_callback_t)(struct dns_cb_data *, query_result **);
...
extern void	dns_queue(struct dns *, void *context, const char *host,
			  enum dns_query_type type, dns_callback_t callback,query_result **q_result);
extern void	dns_cancel(struct dns *, const void *context);
int dns_poll(struct dns *dns, query_result **q_result);
extern int      create_dns_query(char *domain,enum dns_query_type qtype,
                                 struct timeval tv, query_result **res);
//extern query_result *get_result_struct();
extern void     free_query_result(query_result *q_res);
