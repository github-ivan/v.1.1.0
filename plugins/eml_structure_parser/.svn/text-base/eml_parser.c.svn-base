        ...
        if(status==STATUS_HEADERNAME && (start_pointer[count]!=':')){
            count++;
		     ...
		     free(headerN);
                 ...
                 //printf("length %d\n",(start_pointer-mail+1)*sizeof(char));
		...
		if(status==STATUS_PART_TYPE && (start_pointer[count] != '\n' || 
		   ...
		   (status==STATUS_PART_TYPE && start_pointer[count] == '\n' && start_pointer[count+1] == '\r' && start_pointer[count+2] == '\n')){
			...
			if (end_part==NULL){
			    //printf("va a petar\n");
			    wblprintf(LOG_WARNING,"EML PARSER","Email is not well-formed\n");
			    //printf("antes de cambiar el start_pointer\n");
			    start_pointer=body_end;
			    //printf("antes de liberar new type\n");
			    free(new_type);
			    //printf("después de liberar new type\n");
			    //free(boundary);
			    //printf("antes de liberar searching\n");
			    //NOEEEEEEEEEEEEEEEEEEEEEEEEEEEE
			    //free(searching);
			    //printf("después de liberar searching\n");
			    break;
			}
			...
			if (start_pointer<body_end && start_pointer[0]=='-' && start_pointer[1]=='-'){ //The multipart part has ended
			    ...
			    //printf("after pop\n");
			    //printf("before boundary free %s\n",boundary);
			    free(boundary);
			    //printf("after boundary free\n");
			    boundary=NULL;
			    //NOEEEEEEEEEEEEEEEEEEEEEEEEEEEe
			    //free(searching);
			    //printf("after free searching\n");
			    //searching=NULL;
			    break;
			}else{
			    while (start_pointer[0]=='\n' || start_pointer[0]=='\r') start_pointer++;
