...
filelist *list_files(const char *dire,const char *extension){
   ...
   dirp = opendir(dir); /* There should be error handling after this,pendin errno control */
   while ((entry = readdir(dirp)) != NULL) {
     if (entry->d_name[0] != '.'){
	   ...
	   sprintf(confs->list[i],"%s/%s",dir,entry->d_name);
	   i++;
        }
        free(buffer);
     }
...
int count(const char *ruta,const char *ext){
  ...
  dirp = opendir(ruta); /* There should be error handling after this,pendin errno control */
  while ((entry = readdir(dirp)) != NULL) {
